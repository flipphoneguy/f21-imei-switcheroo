...
- **TIQ M5 (offline + hardware confirmed via the Java app port)** — partition dumps analyzed in [`tiq_m5_offline_analysis.md`](tiq_m5_offline_analysis.md). BT_Addr format / WIFI format / trailer-checksum algorithm all match F21 Pro byte-for-byte; `mac_tool.py` round-trips full nvdata.bin byte-identically. Hardware patching confirmed by an end-user running `flipphoneguy/mtk-imei-switcheroo-app` (`MacCrypto.java` is bit-for-bit equivalent to `mac_tool.compute_checksum`, verified offline against the TIQ M5 partition samples). The dump's all-zero WIFI MAC field was the unprovisioned state — the app's patch took effect, resolving the open question from the offline analysis. `live_patch_mac.sh` itself has not been run on TIQ M5 hardware yet, but the underlying algorithm is what got verified.
