...
> **Scope reminder.** Verified end-to-end on hardware for **F21 Pro**, **F25**, and **TIQ M5** via this repo's flows (BT and WiFi MAC). `mac_tool.py` itself has been exercised on TIQ M5 partition samples (round-trip identity) and on TIQ M5 hardware via both the live-patch and offline + `fastboot flash nvdata` paths. The format constants below match what's on F21 Pro's `/vendor/lib64/libnvram.so`; the F25 WIFI header variant (`01 00 09 00`) is supported via `WIFI_HDR_VARIANTS`. On Android 12+ devices (F25 and TIQ M5 in our test set), the WiFi runtime additionally requires invalidating Android's cached factory WiFi MAC in `WifiConfigStore.xml` after the file write — `live_patch_mac.sh` does this; the offline + `fastboot flash` path does not (see [`live_patch_mac.md` § sync_android_wifi_factory_mac](live_patch_mac.md#sync_android_wifi_factory_macnew_mac) for the limitation).
