...
- **TIQ M5** — confirmed via an end-user running `flipphoneguy/mtk-imei-switcheroo-app`. The app's `MacCrypto.java` is a Java port of `mac_tool.py` and is empirically bit-for-bit equivalent on real partition samples (see [Related](#related)) — same `compute_checksum` algorithm, same `BT_Addr` / `WIFI` files patched. `live_patch_mac.sh` itself has not been run on TIQ M5 hardware yet; the underlying algorithm is what got verified.
